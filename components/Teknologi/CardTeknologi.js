...
import styles from './CardTeknologi.module.css'
import { fetchDataAllPost } from '@/pages/api/ghostConfig';

export default function CardTeknologi() {
        ...
        async function fetchData() {
            try {
                ...
                const filteredPosts = data.posts.filter(post => (
                    post.primary_tag && post.primary_tag.name === "Teknologi"
                ));
                    ...
                    <div key={post.id} className={`${styles.iniCols} col-md-4`}>
                        <div className={`card-group `}>
                            <div className={`${styles.iniCard} card h-100`}>
                                <Image src={post?.feature_image} height="1920" width="1920" className={`${styles.iniImages} card-img-top img-fluid`} alt="..." />
                                ...
                                <div className={`d-flex justify-content-end mb-5`}>
                                    <button type="button" className={`${styles.iniButton} btn`}>Baca Sekarang</button>
                                </div>
                            </div>
                        </div>
                    </div>
